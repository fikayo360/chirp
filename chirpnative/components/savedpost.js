  ...
  const windowWidth = Dimensions.get('window').width;
    return (
      (
        ...
        <Text style={[styles.wallheaderTxt,{fontSize:windowWidth*0.055,marginLeft:windowWidth*0.06}]}>{props.data.SavedPostAuthor}</Text>
        </View>
        <View style={[{height:windowWidth * 0.9}]}>
        <Image resizeMode='cover' style={{ width: '100%', height: '100%' }} 
        source={{uri:props.data.SavedPostImg}}/>
        </View>
        <Text style={[styles.title,{fontSize:windowWidth * 0.05, marginBottom:windowWidth*0.02 }]}>{props.data.SavedPostTitle}</Text>
        ...
        <View style={[styles.footer,{width:'100%',height:windowWidth * 0.12,paddingLeft:windowWidth * 0.005,paddingHorizontal:windowWidth*0.7}]}>
          <Text style={[styles.timestamp,{fontSize:windowWidth * 0.04}]}>{[props.data.createdAt.slice(0,10)]}</Text>
